...
export async function generateMetadata({ params }: PageProps) {
  ...
  return { title: `${category.name} | Ludoteca Chilena` };
}
    ...
    <div className="py-10 sm:py-14">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <nav className="mb-6 flex items-center gap-2 text-sm text-[var(--color-text-muted)]">
          ...
          <span>/</span>
          <Link href="/categorias" className="hover:text-[var(--color-brand-blue)] transition-colors">Categorías</Link>
          <span>/</span>
          <span className="text-[var(--color-brand-blue)]">{category.name}</span>
        </nav>
        ...
        <p className="text-[var(--color-text-secondary)] mb-8">
          {games.length} {games.length === 1 ? 'juego encontrado' : 'juegos encontrados'} en esta categoría.
        </p>
        ...
        {games.length > 0 ? (
          ...
          <p className="text-[var(--color-text-muted)] italic">Aún no hay juegos registrados en esta categoría.</p>
        )}
      </div>
    </div>
  );
