...
function YearGroup({
  ...
  return (
    <div className="relative">
      ...
      <YearItems items={items} label={label} />
    </div>
  );
}

const GAMES_INLINE_THRESHOLD = 3;
...
function YearItems({
  ...
  const games = items.filter((i): i is GameItem => i.kind === "game");
  const collapseGames = games.length > GAMES_INLINE_THRESHOLD;

  return (
    <>
      {events.length > 0 && (
        <ul className={`space-y-3 ${games.length > 0 ? "mb-4" : ""}`}>
          {events.map((item) => (
      ...
      {games.length > 0 &&
        (collapseGames ? (
          <details className="group">
            <summary
              className="cursor-pointer select-none list-none [&::-webkit-details-marker]:hidden inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold transition-colors hover:bg-[var(--color-brand-blue-pale)]"
              style={{
                color: "var(--color-brand-blue)",
            ...
            >
              <span>
                Ver {games.length} juegos publicados en {label}
              </span>
              <svg
